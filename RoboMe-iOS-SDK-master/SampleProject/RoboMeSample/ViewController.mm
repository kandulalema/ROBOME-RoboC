...
//#import "ColorCircleViewController.mm"
...
//@property (nonatomic, strong) ColorCircleViewController *colorCircleViewControl;
...
- (void)viewDidLoad {
    ...
    //create Command Player object
    self.commandPlayer = [[CommandPlayer alloc]init];
    
    // start listening for events from RoboMe
    [self.roboMe startListening];
    
    //Creating the object for the ColorCircle for going
   // _colorCircleViewControl = [[ColorCircleViewController alloc]init];
...
- (void)perform:(NSString *)command {
    ...
    else if ([cmd isEqualToString:@"STOP SINGING"]){
        NSLog(@"Inside the stop singing");
        [self.commandPlayer stopPlaying];
        if ([self.commandPlayer isPlaying]== YES) {
            NSLog(@"inside the if block stop sending");
            [self.commandPlayer stopPlaying];
        }
    }
}
